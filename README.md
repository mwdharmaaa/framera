...
- **36 Curated Studio Templates**:
  ...
  - `ambient_duo_card`: Atmospheric darkened full-bleed backdrop paired with floating Spotify photo card, vector headphones, fingerprint heart, and sparkle stars.
  ...
  - `whatsapp_chat_trio`: Viral desktop dark messenger aesthetic with 3 stacked photo messages, forward telemetry, timestamps, and chat input bar.
  - `photobooth_strip`: Editorial split composition featuring full-height hero portrait and vertical 4-frame vintage photobooth parchment strip.
  - `life_memories_quad`: Aesthetic 4-column rounded vertical window slits with 3D block lettering and distributed editorial sub-phrases.
  - `ios_share_story`: Aesthetic 6-photo layout combining hero backdrop, 2 floating framed cards, and bottom iOS share drawer.
  - `kraken_eyes`: Dark noir vertical letterbox slit framed by anime kraken tentacles with suction cups and floating violet sakura petals.
  - `bnw_duo_prints`: Two stacked vintage black and white photographic prints with worn paper borders and silver gelatin analog film tone.
  - `bloom_alone`: Streetwear urban bloom poster with chunky bubbly typography, Hope Gang parental advisory badge, and lone yellow flower accent.
